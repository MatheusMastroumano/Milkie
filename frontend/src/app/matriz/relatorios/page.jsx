...
export default function RelatoriosFinanceiros() {
  ...
  const prepareTrendData = () => {
    ...
    const data = dates.map(date => {
      const entradasDia = entradas.filter(e => e.data === date).reduce((sum, e) => sum + parseFloat(e.valor), 0);
      const saidasDia = saidas.filter(s => s.data === date).reduce((sum, s) => sum + parseFloat(s.valor), 0);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#F9FAFB] via-[#E5E7EB] to-[#F9FAFB]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-[#FFFFFF] bg-opacity-50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-[#E5E7EB]">
          <h3 className="text-xl font-semibold text-[#1F2937] mb-6 text-center">Relatório por Loja</h3>
