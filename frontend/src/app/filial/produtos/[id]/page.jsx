...
export default function ProductDetailsFilial() {
      ...
      <>
        ...
        <main className="min-h-screen flex items-center justify-center bg-[#FFFFFF] pt-16">
          <div className="text-center max-w-md p-6 bg-[#F7FAFC] rounded-lg shadow">
            ...
            <button
              onClick={() => router.push("/filial/produtos")}
  ...
  if (produto.fornecedores_ids) {
    try {
      const ids = Array.isArray(produto.fornecedores_ids) ? produto.fornecedores_ids : JSON.parse(produto.fornecedores_ids);
      if (Array.isArray(ids) && ids.length > 0) {
        // Se temos os fornecedores completos na resposta
        if (produto.fornecedores && Array.isArray(produto.fornecedores)) {
          fornecedoresNomes = produto.fornecedores.map(f => f.nome || f.fornecedor?.nome).filter(Boolean);
        } else {
